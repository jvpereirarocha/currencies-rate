...
from currencies.services.currency_service import (
    CurrencyService,
    GetAllCurrenciesService,
)
from currencies.dependency_injection import DependencyInjection
from currencies.requests.currency_request import (
    CurrencyRequest,
...
@DependencyInjection(
    input_request=GetAllCurrenciesRequest, repository=CurrencyRepo, response=Response
)
def get_all_currencies(request):
    repository = CurrencyRepo()
    get_all_currencies_service = GetAllCurrenciesService(
        request=GetAllCurrenciesRequest(), repo=repository
    )
    get_all_currencies_service.get_all()
    service_response: ResponseData = get_all_currencies_service.make_response()
